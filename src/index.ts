...
import {
  commandConsoleInterpreter,
  createCommandConsoleTemplate,
} from "./command-console/index.js";
...
const handleEvalResult = (ctx: any, result: any) => {
  const timestamp =
...
const terminalActivation = async (ctx: any) => {
...
const terminalDeactivation = async (ctx: any) => {
...
const terminalEvaluation = async (ctx: any) => {
  ...
  try {
    let result: any = ctx.interpreter(message, ...args);
...
const scrollTerminalBottom =
  ({ console }: any) =>
  () =>
    console && (console.scrollTop = console.scrollHeight);

const inputFocusing = async (ctx: any) => {
  console.log("focus input", ctx.terminalInput);
  if (ctx.terminalInput) ctx.terminalInput.focus();
  setTimeout(scrollTerminalBottom(ctx), 10);
};

const inputClearing = async (ctx: any) => {
  if (ctx.terminalInput) ctx.terminalInput.value = "";
  return { input: "" };
};

const errorTerminal = async (ctx: any) => {
  if (ctx.terminalInput !== null) {
    if (ctx.error) ctx.output.push({ display: ctx.error, type: "error" });
...
// Reductions
const reduceNextHistory = reduce((ctx: any, evt: any) => {
...
const reducePrevHistory = reduce((ctx: any, evt: any) => {
  const history = ctx.history;
  const historyIndex = ctx.historyIndex - 1;
...
const terminalMachine = createMachine(
  "closed",
  {
    ...
    focused: state(
      transition("typing", "focused", reduceWithKeys("input", "message")),
      transition(
        "next-history",
        "focused",
        guardKeyZero("historyIndex"),
        reduceNextHistory
      ),
      transition("prev-history", "focused", reducePrevHistory),
      transition("eval", "eval", reduceWithKeys("input", "message")),
      transition("focus", "focus"),
      transition("close-console", "deactivating"),
      transition("error", "error", reduceWithKeys("error", "error"))
    ),
    "clear-terminal": state(
      immediate("clear", reduceSetKeyValue("output", []))
    ),
...
const handlePromptKeys = (e: KeyboardEvent) => {
  ...
  if (key === "Enter") terminalMachineService.send({ type: "eval", message });
  else if (key === "Escape") terminalMachineService.send("close-console");
  else if (key === "ArrowUp")
    terminalMachineService.send({ type: "next-history" });
  else if (key === "ArrowDown")
    terminalMachineService.send({ type: "prev-history" });
  else terminalMachineService.send({ type: "typing", message });
};

const consoleTemplate = createCommandConsoleTemplate(
  TERMINAL_ID,
  PROMPT_ID,
  TERMINAL_OUTPUT_ID,
  store,
...
document.body.addEventListener("keyup", (e: KeyboardEvent) => {
  ...
  if (!command) return;
  if (current === "closed") {
    if (command === "go-back") return window.history.go(-1);
    if (command !== "activate") return;
  } else {
    if (command === "activate") command = "focus";
    if (command === "go-back") return;
  }
