...
const msgNext = (next: string, message: string, log?: string) => (
  log && console.log(log),
...
const COMMANDS = {
  ...
  close: () =>
    msgNext("close-console", "Closed console", "Closing command console"),
  start: (app, ...args) => {
    console.log('Starting "app":', app, args);
    if (app in APPS) return APPS[app](...args);
    else return msgNext("error", `Cannot start unknown app: ${app}`);
  },
  stop: app => {
    ...
    } else return msgNext("error", `Cannot stop unknown app: ${app}`);
  },
  goto: link => {
    if (LINKS[link]) {
    ...
    if (link.startsWith("http")) {
      ...
      try {
        ...
        return msgNext(
          "error",
          `Link "${link}" does not seem to be a valid URL`
        );
      }
    }
  },
...
// Interpreters
export const commandConsoleInterpreter = (message: string, ...args) => {
